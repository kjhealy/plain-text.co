...
```{r owl, fig.cap='How to Draw an Owl.', echo=FALSE, out.width="100%"}
knitr::include_graphics("figures/draw-owl.jpg")
```
...
them with `pandoc`, tracking things with Git and using (behind the
...
[TextMate 2](http://macromates.com/download) (shown in Figure \@ref(fig:tm). On
...
```{r tm, fig.cap='Part of an R file being edited in TextMate.', echo=FALSE, out.width="100%", fig.fullwidth=TRUE}
...
Figure \@ref(fig:rstudio). Although it appears quite late in this discussion, it
...
```{r rstudio, fig.cap='RStudio running on Windows.', echo=FALSE, out.width="100%"}
...
It would be nice if all you needed to do your work was a box 
